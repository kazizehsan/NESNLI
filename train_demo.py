...
small_train_dataset = tokenized_datasets["train"].shuffle(seed=42).select(range(640))
small_eval_dataset = tokenized_datasets["test"].shuffle(seed=42).select(range(40))
...
model_output_dir = "nesnli-small-trainer"

# ## for 320 train samples
...
#     eval_steps=10, # Number of update steps between two evaluations, for use with #evaluation_strategy="steps"
#     logging_steps=10, # logging happens at eval_steps if logging_steps < eval_steps
    
#     #num_train_epochs=3,
#     max_steps=20, # if set there will be only 1 epoch
...
#     save_steps=10, # Number of update steps between two saves. Reduce if each step is taking too long.
# )

## for 640 train samples
training_args = TrainingArguments(
    model_output_dir,
    ...
    eval_accumulation_steps=5, # Number of predictions steps before moving output tensors to CPU
    evaluation_strategy="steps",
    eval_steps=20, # Number of update steps between two evaluations, for use with #evaluation_strategy="steps"
    logging_steps=20, # logging happens at eval_steps if logging_steps < eval_steps
    
    #num_train_epochs=3,
    max_steps=40, # if set there will be only 1 epoch
    # but if gradient_accumulation_steps is also set, training wont stop early even if dataset is exhausted,
    # it will complete all max_steps. in fact, in this case, the number of epochs will increase
    ...
    save_steps=20, # Number of update steps between two saves. Reduce if each step is taking too long.
)
